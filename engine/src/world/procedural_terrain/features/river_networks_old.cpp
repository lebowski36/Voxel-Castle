...
// This system generates connected river paths that flow from sources to destinations
// Uses deterministic algorithms that can be computed per-region and cached

// Simple noise function for deterministic generation
...
// Get terrain elevation using existing multi-scale noise system
static float GetElevation(float worldX, float worldZ, uint64_t seed) {
    // Use the actual terrain generation system for consistent elevation
    float continental = MultiScaleNoise::GenerateNoise(worldX, worldZ, TerrainScale::CONTINENTAL, seed) * 1400.0f;
    float regional = MultiScaleNoise::GenerateNoise(worldX, worldZ, TerrainScale::REGIONAL, seed + 1000) * 500.0f;
    float local = MultiScaleNoise::GenerateNoise(worldX, worldZ, TerrainScale::LOCAL, seed + 2000) * 120.0f;
    float micro = MultiScaleNoise::GenerateNoise(worldX, worldZ, TerrainScale::MICRO, seed + 3000) * 30.0f;
    ...
    return base_elevation + local + micro;
}

// === RIVER NETWORK DATA STRUCTURES ===

struct RiverSource {
    float x, z;           // World coordinates of source
    float elevation;      // Elevation at source
    float flow_strength;  // Initial flow strength
    int river_id;         // Unique identifier
};

struct RiverPath {
    std::vector<glm::vec2> points;  // Path points from source to destination
    float flow_strength;            // Flow strength (increases downstream)
    int river_id;                   // Unique identifier
    int parent_river_id;            // -1 if main river, else parent river ID
    bool ends_in_lake;              // True if river ends in a lake
    bool ends_in_ocean;             // True if river flows to ocean
};

struct Lake {
    float center_x, center_z;       // Lake center
    float radius;                   // Lake radius
    float elevation;                // Lake surface elevation
    int lake_id;                    // Unique identifier
    std::vector<int> inlet_rivers;  // Rivers flowing into lake
    std::vector<int> outlet_rivers; // Rivers flowing out of lake
};

// Regional river network cache
struct RegionRiverData {
    std::vector<RiverPath> rivers;
    std::vector<Lake> lakes;
    float region_x, region_z;      // Region center coordinates
    bool is_generated;
};

// Global cache for river networks (1km x 1km regions)
static std::unordered_map<uint64_t, RegionRiverData> river_cache;
...
float RiverNetworks::CalculateFlowAccumulation(float worldX, float worldZ, uint64_t seed) {
    ...
    // Even more realistic thresholds - sparse river networks
    // Continental drainage (major rivers) - extremely rare (1% of land)
    if (continental > 0.99f) {
        flow_accumulation += 1000.0f; // Base flow for major rivers
        
        // Major rivers collect from all smaller systems in their watershed
        if (regional > 0.2f) flow_accumulation += 200.0f;
        if (local > 0.1f) flow_accumulation += 40.0f;
        if (streams > 0.05f) flow_accumulation += 8.0f;
        if (creeks > 0.05f) flow_accumulation += 1.0f;
    }
    // Regional drainage (tributaries) - very uncommon (3% of remaining land)
    else if (regional > 0.97f) {
    ...
    // Local drainage (small rivers) - uncommon (8% of remaining land)
    else if (local > 0.92f) {
        flow_accumulation += 40.0f; // Base flow for small rivers
        
        if (streams > 0.3f) flow_accumulation += 8.0f;
        if (creeks > 0.1f) flow_accumulation += 1.0f;
    }
    // Stream drainage - moderate (15% of remaining land)
    else if (streams > 0.85f) {
        flow_accumulation += 8.0f; // Base flow for streams
        
        if (creeks > 0.2f) flow_accumulation += 1.0f;
    }
    // Creek drainage - common (20% of remaining land)
    else if (creeks > 0.8f) {
