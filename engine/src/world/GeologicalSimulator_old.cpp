...
void GeologicalSimulator::simulateTectonicPhase(float durationMillionYears) {
    ...
    // Prepare tectonic fields structure
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        // Simulate geological processes using modular engines
        ...
        // Use TectonicEngine for all tectonic processes
        tectonicEngine_->simulateMantleConvection(tectonicFields, timeStep);
        tectonicEngine_->simulatePlateMovement(tectonicFields, timeStep);
        tectonicEngine_->simulateMountainBuilding(tectonicFields, timeStep);
        
        if (step % std::max(1, steps / 4) == 0) {
            tectonicEngine_->simulateVolcanicActivity(tectonicFields, timeStep * 4); // Less frequent volcanic activity
        }
        
        // Isostatic adjustment
        if (step % std::max(1, steps / 8) == 0) {
            tectonicEngine_->simulateIsostasyAdjustment(tectonicFields, timeStep * 8);
        }
...
void GeologicalSimulator::simulateErosionPhase(float durationThousandYears) {
    ...
    // Prepare erosion fields structure
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        // Use WaterSystemSimulator for all water-related processes
        waterSystem_->simulatePrecipitationPatterns(waterFields, timeStep);
        waterSystem_->simulateSurfaceWaterAccumulation(waterFields, timeStep);
        waterSystem_->simulateRiverFormation(waterFields, timeStep);
        waterSystem_->simulateFloodPlains(waterFields, timeStep);
        waterSystem_->simulateLakeFormation(waterFields, timeStep);
        
        // Groundwater processes
        waterSystem_->simulateGroundwaterTable(waterFields, timeStep);
        waterSystem_->simulateAquiferRecharge(waterFields, timeStep);
        waterSystem_->simulateGroundwaterFlow(waterFields, timeStep);
        waterSystem_->simulateSpringFormation(waterFields, timeStep);
        waterSystem_->simulateWetlandFormation(waterFields, timeStep);
        
        // Cave systems (if enabled)
        if (config_.custom.enableCaveSystems) {
            waterSystem_->simulateCaveNetworkGrowth(waterFields, timeStep);
            waterSystem_->simulateUndergroundRivers(waterFields, timeStep);
            waterSystem_->simulateKarstWeathering(waterFields, timeStep);
            waterSystem_->simulateCaveCollapse(waterFields, timeStep);
        }
        
        // Use ErosionEngine for all erosion processes
        erosionEngine_->simulateSedimentTransport(erosionFields, timeStep);
        erosionEngine_->simulateWaterDrivenErosion(erosionFields, timeStep);
        erosionEngine_->simulateErosionUpliftBalance(erosionFields, timeStep);
        erosionEngine_->simulateChemicalWeathering(erosionFields, timeStep * 0.5f);
        erosionEngine_->simulatePhysicalErosion(erosionFields, timeStep * 0.5f);
        erosionEngine_->simulateRiverSystems(erosionFields, timeStep * 0.3f);
        
        if (config_.custom.enableGlacialFeatures && step % std::max(1, steps / 8) == 0) {
            erosionEngine_->simulateGlacialCarving(erosionFields, timeStep * 8); // Occasional glacial periods
        }
...
void GeologicalSimulator::simulateDetailPhase(float durationHundredYears) {
    ...
    // Prepare erosion fields structure for detail processes
    ErosionEngine::ErosionFields erosionFields;
    erosionFields.elevationField = elevationField_.get();
    erosionFields.rockHardness = rockHardness_.get();
    erosionFields.waterFlow = waterFlow_.get();
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        // Use ErosionEngine for detail processes
        erosionEngine_->simulateMicroWeathering(erosionFields, timeStep);
        erosionEngine_->simulateSedimentDeposition(erosionFields, timeStep);
...
bool GeologicalSimulator::stepSimulation() {
    ...
    if (config_.custom.enableCaveSystems) {
        ...
        if (currentStep_ % 4 == 0) { // Every 4th step
            waterSystem_->simulateKarstWeathering(waterFields, baseTimeScales_.waterTimeStep * 4.0f);
            waterSystem_->simulateCaveCollapse(waterFields, baseTimeScales_.waterTimeStep * 4.0f);
        }
    }
    ...
    // === ISOSTATIC ADJUSTMENT ===
    // Run periodically for stability
    if (currentStep_ % 10 == 0) { // Every 10th step
    ...
    if (progressCallback_) {
        ...
        phaseInfo.phaseProgress = totalProgress; // Single interleaved phase
...
void GeologicalSimulator::startBackgroundSimulation() {
    if (backgroundEngine_) {
        ...
        std::cout << "[GeologicalSimulator] Background simulation started" << std::endl;
    }
}
...
void GeologicalSimulator::stopBackgroundSimulation() {
    if (backgroundEngine_) {
        ...
        std::cout << "[GeologicalSimulator] Background simulation stopped" << std::endl;
    }
}

bool GeologicalSimulator::isBackgroundSimulationRunning() const {
    return backgroundEngine_ && backgroundEngine_->isRunning();
}

void GeologicalSimulator::enableBackgroundExecution(bool enable) {
    useBackgroundExecution_ = enable;
    std::cout << "[GeologicalSimulator] Background execution " 
              << (enable ? "enabled" : "disabled") << std::endl;
}

std::shared_ptr<GeologicalSnapshot> GeologicalSimulator::getLatestSnapshot() {
    return consumeLatestSnapshot();
}

float GeologicalSimulator::getBackgroundProgress() const {
    if (totalSteps_ <= 0) return 0.0f;
    return static_cast<float>(currentStep_) / static_cast<float>(totalSteps_);
}

void GeologicalSimulator::createBackgroundSnapshot() {
    // Create a snapshot for background thread consumption
    if (snapshotManager_) {
        float progress = getBackgroundProgress();
        std::string description = "Background Simulation (" + 
                                std::to_string(static_cast<int>(progress * 100.0f)) + "%)";
        createSnapshot(description, progress);
    }
}

std::shared_ptr<GeologicalSnapshot> GeologicalSimulator::consumeLatestSnapshot() {
    // Return the latest snapshot for UI consumption
    if (snapshotManager_) {
        const auto* snapshot = snapshotManager_->GetCurrentSnapshot();
        if (snapshot) {
            // Create a shared pointer copy for safe UI access
            // This is a simplified implementation - in a full version we'd need proper threading
            return std::make_shared<GeologicalSnapshot>(*snapshot);
        }
    }
    return nullptr;
}
