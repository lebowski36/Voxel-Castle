...
void ChunkMesh::addQuad(const glm::vec3& position, CubeFace face, VoxelType blockType) {
    unsigned int baseIndex = vertices.size();
    
    // Get the appropriate texture ID for this face
    int textureId = getTextureIdForFace(blockType, face);
    
    // Calculate texture coordinates based on the specific texture ID
    float u = (textureId % 32) / 32.0f;
    float v = (textureId / 32) / 32.0f;
    float uvStep = 1.0f / 32.0f;

    // ...existing code for normal and vertex calculations...
}