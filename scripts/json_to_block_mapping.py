...
def convert_to_block_mapping(blocks_data: Dict[str, Any]) -> Tuple[Dict[int, Dict[str, str]], Dict[int, str]]:
    ...
    for block_name, block_data in blocks_data.items():
        ...
        if block_id is not None:
            ...
            # Extract face pattern
            face_pattern = texture_info.get('face_pattern', 'UNIFORM')
            face_patterns[block_id] = face_pattern
            
    return block_mapping, face_patterns

def generate_block_mapping_file(block_mapping: Dict[int, Dict[str, str]], face_patterns: Dict[int, str], output_file: str):
    """Generate a Python file with the BLOCK_MAPPING dictionary and face patterns"""
    
    # Sort by block ID for consistent output
    sorted_mapping = dict(sorted(block_mapping.items()))
    sorted_patterns = dict(sorted(face_patterns.items()))
...
'''
    ...
    # Add face patterns
    content += '''# Face patterns for blocks (GENERATED)
# These should match the patterns in atlas_face_system.py
GENERATED_FACE_PATTERNS = {
'''
    
    for block_id, pattern in sorted_patterns.items():
        content += f"    {block_id}: '{pattern}',\n"
...
def compare_mappings(old_mapping: Dict[int, Dict[str, str]], new_mapping: Dict[int, Dict[str, str]]):
    ...
    print("\n🔍 Comparing BLOCK_MAPPING formats...")
    ...
    # Report results
    print(f"   ✅ Matching entries: {matches}")
    
    if missing_in_new:
        print(f"   ❌ Missing in JSON data: {len(missing_in_new)} blocks")
        for block_id in missing_in_new[:5]:  # Show first 5
            print(f"      • Block ID {block_id}: {old_mapping[block_id]}")
        if len(missing_in_new) > 5:
            print(f"      • ... and {len(missing_in_new) - 5} more")
    
    if missing_in_old:
        print(f"   📝 New in JSON data: {len(missing_in_old)} blocks")
        for block_id in missing_in_old[:5]:  # Show first 5
            print(f"      • Block ID {block_id}: {new_mapping[block_id]}")
        if len(missing_in_old) > 5:
            print(f"      • ... and {len(missing_in_old) - 5} more")
    
    if differences:
        print(f"   ⚠️  Different entries: {len(differences)}")
        for diff in differences[:3]:  # Show first 3
            print(f"      • Block ID {diff['id']}:")
...
def main():
    ...
    try:
        print("🔄 Converting JSON block data to BLOCK_MAPPING format...")
        
        # Load unified block data
        blocks_data = load_unified_block_data(data_dir)
        print(f"   📖 Loaded {len(blocks_data)} blocks from JSON data")
        
        # Convert to legacy format
        block_mapping, face_patterns = convert_to_block_mapping(blocks_data)
        print(f"   🔄 Converted {len(block_mapping)} entries to BLOCK_MAPPING format")
        print(f"   🎭 Extracted {len(face_patterns)} face patterns")
        
        # Generate output file
        generate_block_mapping_file(block_mapping, face_patterns, output_file)
        ...
        if current_mapping:
            ...
            if is_compatible:
                print("\n✅ JSON data is fully compatible with current BLOCK_MAPPING!")
            else:
                print("\n⚠️  JSON data has differences from current BLOCK_MAPPING")
