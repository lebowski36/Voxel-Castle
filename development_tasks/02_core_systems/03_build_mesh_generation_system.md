...
### 2. Mesh Optimization (e.g., Greedy Meshing)
    ...
    - [ ] 2.4. **Testing:**
        - [ ] 2.4.1. Create a test `ChunkSegment` with a variety of voxel configurations (e.g., solid blocks, flat planes, hollow shapes, single voxels, mixed types).
        - [ ] 2.4.2. Call `buildGreedyMesh` on the test chunk.
        - [ ] 2.4.3. Compare the number of vertices/indices generated by `buildGreedyMesh` vs. `buildNaiveMesh` for the same chunk. Expect significantly fewer for `buildGreedyMesh`.
